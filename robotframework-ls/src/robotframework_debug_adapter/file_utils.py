...
try:
    PATHS_FROM_IDE_TO_PYTHON = json.loads(
        os.environ.get("PATHS_FROM_IDE_TO_PYTHON", "[]")
    )
except Exception:
    log.critical("Error loading PATHS_FROM_IDE_TO_PYTHON from environment variable.")
    log.exception()
    PATHS_FROM_IDE_TO_PYTHON = []
else:
    if not isinstance(PATHS_FROM_IDE_TO_PYTHON, list):
        log.critical(
            "Expected PATHS_FROM_IDE_TO_PYTHON loaded from environment variable to be a list."
        )
        PATHS_FROM_IDE_TO_PYTHON = []
    else:
        # Converting json lists to tuple
        PATHS_FROM_IDE_TO_PYTHON = [tuple(x) for x in PATHS_FROM_IDE_TO_PYTHON]
            ...
            try:
                ...
                try:
                    ...
                    if os_path_exists(filename):
                        ...
                        log.critical(
                            "robot debugger: critical: unable to get real case for file. Details:\n"
                            "filename: %s\ndrive: %s\nparts: %s\n"
...
def set_ide_os(os):
    ...
    if DEBUG_CLIENT_SERVER_TRANSLATION:
        print("robot debugger: client OS: %s" % (os,))
...
def setup_client_server_paths(paths):
    """paths is the same format as PATHS_FROM_IDE_TO_PYTHON"""
    ...
    paths_from_IDE_to_python = initial_paths[:]
    ...
    for i, (path0, path1) in enumerate(paths_from_IDE_to_python[:]):
        ...
        paths_from_IDE_to_python[i] = (_normcase_from_client(path0), normcase(path1))

    if not paths_from_IDE_to_python:
    ...
    def _norm_file_to_server(filename, cache=norm_filename_to_server_container):
        ...
        try:
            ...
            for eclipse_prefix, server_prefix in paths_from_IDE_to_python:
                if translated_normalized.startswith(eclipse_prefix):
                    found_translation = True
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        log.critical(
                            "robot debugger: replacing to server: %s", filename
                        )
                    translated = server_prefix + filename[len(eclipse_prefix) :]
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        log.critical("robot debugger: sent to server: %s", translated)
                    break
            else:
            ...
            if found_translation:
                ...
                if not os_path_exists(translated):
                    if not translated.startswith("<"):
                        ...
                        log.error_once(
                            'robot debugger: unable to find translation for: "%s" in [%s] (please revise your path mappings).\n',
                            filename,
                            ", ".join(
                                ['"%s"' % (x[0],) for x in paths_from_IDE_to_python]
                            ),
                        )
                else:
        ...
        try:
            ...
            if IS_WINDOWS:
                if translated.lower() != translated_proper_case.lower():
                    ...
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        log.critical(
                            "robot debugger: _NormFile changed path (from: %s to %s)",
            ...
            for i, (eclipse_prefix, python_prefix) in enumerate(
                ...
                if translated.startswith(python_prefix):
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        log.critical(
                            "robot debugger: replacing to client: %s", translated
                        )
                    ...
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        log.critical("robot debugger: sent to client: %s", translated)
                    path_mapping_applied = True
                    break
            else:
                if DEBUG_CLIENT_SERVER_TRANSLATION:
                    log.critical(
                        "robot debugger: to client: unable to find matching prefix for: %s in %s",
                        translated,
                        [x[1] for x in paths_from_IDE_to_python],
                    )
...
setup_client_server_paths(PATHS_FROM_IDE_TO_PYTHON)
